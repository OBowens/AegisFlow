# NOT WIRED UP (run_log_analysis_pipeline only) -- placeholder/design-sketch
# only, has zero callers, and builds an in-memory dict that is never
# persisted to the database; the apps.ai_core.modules.* stages it calls are
# themselves static placeholders incompatible with the current model schema
# (see the 2026-08-24 investigation). Do not extend without re-reading that
# note first. This does NOT apply to run_demo_log_workflow below, in this
# same file -- that function is the real, live pipeline actually called
# from apps/log_intake/views.py and does not touch apps.ai_core.modules.
from apps.audit.models import AIRun, AuditLog
from apps.incidents.services.correlation import link_source_ip_matches_for_incidents
...
from apps.reports.services.generator import create_upload_summary_report_for_upload
...
def run_demo_log_workflow(uploaded_log_file: UploadedLogFile) -> dict:
    ...
    try:
        ...
        link_source_ip_matches_for_incidents(incidents)
        ...
        report = create_upload_summary_report_for_upload(uploaded_log_file)
