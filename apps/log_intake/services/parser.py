...
# Source types with a dedicated, format-aware parser. Everything else
# (currently just "other") still falls through to the generic
# extension-based parsing below because no dedicated parser exists for it.
DEDICATED_PARSERS = {
    UploadedLogFile.SourceType.LINUX: parse_linux_auth_log,
    UploadedLogFile.SourceType.WAZUH: parse_wazuh_log,
    UploadedLogFile.SourceType.GRAYLOG: parse_graylog_log,
    UploadedLogFile.SourceType.WINDOWS: parse_windows_security_log,
    UploadedLogFile.SourceType.PRTG: parse_prtg_log,
    UploadedLogFile.SourceType.SSL_CERTIFICATE: parse_ssl_certificate_log,
    UploadedLogFile.SourceType.FIREWALL: parse_fortigate_firewall_log,
    UploadedLogFile.SourceType.BACKUP: parse_backup_report_log,
    UploadedLogFile.SourceType.PROXMOX: parse_proxmox_syslog,
}
...
def parse_uploaded_log(uploaded_log_file: UploadedLogFile) -> list[ParsedAlert]:
    ...
    try:
        ...
        dedicated_parser = DEDICATED_PARSERS.get(uploaded_log_file.source_type)

        if dedicated_parser:
            events = dedicated_parser(raw_text, source_tool=uploaded_log_file.source_type)
            return [
